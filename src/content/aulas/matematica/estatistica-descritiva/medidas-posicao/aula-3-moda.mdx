...
<summary>MEDIDAS DE TENDÊNCIA CENTRAL</summary>
    ...
    .t .moda{border-color:#4a9eff}
    .t .moda div{border-bottom-color:#4a9eff;color:#4a9eff}
    .t .moda>div:first-child{background:#1a3a5e;color:#4a9eff}
    .t .moda::before{background:#4a9eff}
    </style>
    <div class="t">
...
### 2. Moda para Dados Brutos (Não Agrupados)
...
<details>
<summary>📊 Exemplo da Moda para Dados Não Agrupados no Excel</summary>
...
> **⚠️ Atenção:** A função `MODO` no Excel retorna **apenas o menor valor** entre os que têm maior frequência. Para ver **todas as modas** (4 e 6), é preciso usar a função `MODO.MULT` (ou `MODE.MULT` em inglês), que retorna uma matriz.

**Passo 3 — Calcular todas as modas com MODO.MULT:**

Selecione duas células vazias (ex.: C1:C2), digite:

```
=MODO.MULT(A2:A10)
```

E pressione **Ctrl + Shift + Enter** (para fórmulas matriciais em versões antigas do Excel).
...
|--------|--------|-----------|-----------|
| Moda única | `=MODE(A2:A8)` | `=MODO(A2:A8)` | 9 |
| Todas as modas | `=MODE.MULT(A2:A10)` | `=MODO.MULT(A2:A10)` | 4 e 6 |

> **💡 Dica:** Use `MODO` quando quiser apenas uma resposta rápida. Use `MODO.MULT` quando precisar verificar se o conjunto é bimodal ou multimodal.

</details>

<details>
<summary>🐍 Exemplo da Moda para Dados Não Agrupados no Python</summary>
...
- Opção 3 — Função direta (pandas)
```python
import pandas as pd

notas = [7, 8, 8, 9, 9, 9, 10]
moda = pd.Series(notas).mode()
print(moda)
```
</details>

<details>
<summary>📊 Exemplo da Moda para Dados Não Agrupados no R</summary>
...
```r
notas <- c(7, 8, 8, 9, 9, 9, 10)
moda <- names(sort(table(notas), decreasing = TRUE))[1]
print(moda)
```
...
<details>
<summary>🐍 Exemplo da Moda para Dados Agrupados por Lista no Python</summary>
...
### 4. Moda para Dados Agrupados por Intervalo de Classe
...
**4.1 Classe Modal (sem interpolação)**
...
**4.2 Moda Interpolada — Fórmula de Czuber**
...
### 5. Vantagens e Desvantagens da Moda
...
### 6. Quando Usar — Média × Mediana × Moda
...
> Em **dados brutos**, a moda é o valor que mais se repete.<br>
 Em **dados agrupados por lista**, é o valor com maior frequência na tabela.<br>

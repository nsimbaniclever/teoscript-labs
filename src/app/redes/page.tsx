...
function GlobalNetwork({ color = '#06b6d4' }: { color?: string }) {
...
export default function RedesPage() {
  const part = parts.find(p => p.id === 'redes')!
    ...
    <div className="min-h-screen relative overflow-hidden" style={PAPEL}>
          ...
          return (
            <motion.span
              ...
              style={{
                ...
                textShadow: '0 0 50px rgba(6,182,212,0.1)',
                filter: 'blur(0.3px)',
              }}
              animate={{
                x: xMovement,
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 md:px-6 py-6">
        <div className="flex items-center justify-between mb-8">
          <Link
            href="/"
            className="inline-flex items-center gap-2 text-sm font-semibold px-5 py-2.5 rounded-full text-white hover:-translate-y-0.5 transition-all shadow-lg hover:shadow-[#06b6d4]/30"
            style={{ background: `linear-gradient(135deg, ${cor}, #0891b2)` }}
          >
            <ArrowLeft size={16} /> Voltar
          </Link>

          <div className="hidden md:flex items-center gap-2 pb-1 border-b-2 transition-all duration-300 hover:border-[#06b6d4]/50 group" style={{ borderBottomColor: 'rgba(6,182,212,0.15)' }}>
            <motion.div 
              className="relative w-8 h-8 rounded-xl flex items-center justify-center shadow-lg transition-all duration-300 group-hover:scale-110 group-hover:shadow-[0_0_40px_rgba(6,182,212,0.6)]"
              style={{
                background: `linear-gradient(135deg, ${cor}, #0891b2)`,
              }}
            ...
            >
              ...
              <motion.div 
                ...
                animate={{
                  boxShadow: ['inset 0 0 0px rgba(6,182,212,0)', 'inset 0 0 20px rgba(6,182,212,0.3)', 'inset 0 0 0px rgba(6,182,212,0)'],
                }}
                transition={{ duration: 2, repeat: Infinity }}
              />
            </motion.div>
            <div>
              <span className="font-bold text-[#67e8f9] text-sm tracking-tight flex items-center gap-1">
                TeoScript <span style={{ color: '#ffffff' }}>Labs</span>
              </span>
              <span className="block text-[8px] font-medium text-[#67e8f9]/60 tracking-[0.25em] uppercase leading-none">
                Doctorate · Redes
              </span>
            </div>
          </div>
        ...
        <motion.div 
          className="relative bg-[#050a0f]/90 backdrop-blur-sm rounded-2xl border border-[#06b6d4]/20 shadow-lg shadow-[#06b6d4]/5 overflow-hidden mb-8"
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
        >
          <div className="h-1 w-full" style={{ background: 'linear-gradient(to right, #06b6d4, #67e8f9, #0891b2, #06b6d4)' }} />
          
          <div className="absolute top-0 right-0 w-96 h-96 bg-[#06b6d4]/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 animate-pulse" />
          <div className="absolute bottom-0 left-0 w-96 h-96 bg-[#0891b2]/5 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2 animate-pulse" style={{ animationDelay: '1s' }} />
          ...
          <div className="px-6 md:px-12 py-8 md:py-12 text-center relative">
            ...
            <div className="flex flex-wrap justify-center gap-4 mt-6 text-sm">
              <motion.span 
                className="px-4 py-1.5 bg-[#06b6d4]/10 rounded-full border border-[#06b6d4]/30 text-[#67e8f9]"
        ...
        <div className="space-y-4">
          {part.sections.map((subtema, temaIndex) => {
            ...
            return (
              <motion.div 
                key={subtema.id} 
                className="bg-[#050a0f]/90 rounded-xl border border-[#06b6d4]/10 overflow-hidden shadow-sm hover:shadow-md hover:border-[#06b6d4]/30 transition-all"
              ...
              >
                <button
                  ...
                  className="w-full flex items-center justify-between gap-4 p-5 hover:bg-[#06b6d4]/5 transition-colors group"
                  ...
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div className="text-left flex-1 min-w-0">
                      <h3 className="text-lg font-semibold" style={{ color: cor }}>
                        {temaIndex + 1}. {subtema.title}
                      </h3>
                      <span className="text-sm text-[#67e8f9]/40">
                        {subtema.topics.length} tópicos
                      </span>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <span className="text-sm font-medium text-[#67e8f9]/40 hidden sm:inline">
                      {isOpen ? 'Recolher' : 'Expandir'}
                    </span>
                    <motion.div
                      animate={{ rotate: isOpen ? 180 : 0 }}
                      transition={{ duration: 0.3 }}
                      className="text-[#67e8f9]/40 group-hover:text-[#67e8f9] transition-colors"
                    >
                      <ChevronDown size={20} />
                    </motion.div>
                  </div>
                ...
                <AnimatePresence>
                  {isOpen && (
                    ...
                    >
                      <div className="p-4 md:p-6 border-t border-[#06b6d4]/10 space-y-3 bg-[#050a0f]/90">
                        {subtema.topics.map((sub) => {
                          const isSubOpen = subsubtemasAbertos.includes(sub.id)
                          const aulasData = getAulasFromDoctorate(sub.id)

                          return (
                            <div key={sub.id} className="border border-[#06b6d4]/10 rounded-lg overflow-hidden hover:border-[#06b6d4]/30 transition-all bg-[#050a0f]/90">
                              <button
                                onClick={() => toggleSubsubtema(sub.id)}
                                className="w-full flex items-center justify-between gap-4 p-4 hover:bg-[#06b6d4]/5 transition-colors group"
                              >
                                <div className="flex items-center gap-3">
                                  ...
                                  <span className="font-medium text-[#67e8f9] group-hover:text-[#06b6d4] transition-colors">
                                    {sub.title}
                                  </span>
                                  <span className="text-xs text-[#67e8f9]/40">
                              ...
                              <AnimatePresence>
                                {isSubOpen && (
                                  <motion.div
                                    ...
                                    className="overflow-hidden"
                                  >
                                    <div className="p-4 border-t border-[#06b6d4]/10 overflow-x-auto bg-[#050a0f]/90">
                                      <table className="w-full text-sm border-collapse">
                                        <thead>
                                          <tr style={{ background: `${cor}15` }}>
                                            <th className="px-4 py-2 text-left font-semibold text-[#67e8f9] w-[22%]">Aula</th>
                                            <th className="px-4 py-2 text-left font-semibold text-[#67e8f9] w-[28%]">O que estudar</th>
                                            <th className="px-4 py-2 text-left font-semibold text-[#67e8f9] w-[28%]">Aplicação na Tese</th>
                                            <th className="px-4 py-2 text-left font-semibold text-[#67e8f9] w-[22%]">Projeto Sugerido</th>
                                          </tr>
                                        </thead>
                                        <tbody>
                                          {aulasData.map((aula, idx) => (
                                            <tr
                                              key={idx}
                                              className={`border-b border-[#06b6d4]/5 hover:bg-[#06b6d4]/5 transition-colors ${
                                                idx % 2 === 0 ? 'bg-[#06b6d4]/5' : 'bg-transparent'
                                              }`}
                                            >
                                              <td className="px-4 py-2 font-medium text-[#67e8f9] align-top">
                                                <div className="flex items-center gap-2 flex-wrap group">
                                                  ...
                                                  >
                                                    <Link
                                                      href={`/aula/${aula.slug}`}
                                                      className="inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full text-[#050a0f] transition-all duration-300 opacity-0 group-hover:opacity-100 hover:-translate-y-0.5 whitespace-nowrap"
                                                      style={{ background: cor }}
                                                    >
                                              ...
                                              <td className="px-4 py-2 text-[#67e8f9]/60 align-top text-xs">
                                                {aula.estudo}
                                              </td>
                                              <td className="px-4 py-2 text-[#67e8f9]/60 align-top text-xs">
                                                {aula.tese}
